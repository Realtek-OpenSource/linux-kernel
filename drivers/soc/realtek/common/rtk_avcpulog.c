...
#define HWSEM_TIMEOUT		500 /* msec */
#define WORK_DELAY_TIME		500 /* msec */
...
avlog_ctl_t *avlog_ctl_g = NULL;

/* return value 0 means success getting semaphore with preempt disabled */
static inline int avlog_hwsem_get(struct sb2_sem *sem)
{
	unsigned long timeout;

	timeout = jiffies + msecs_to_jiffies(HWSEM_TIMEOUT);
...
static int __init rtk_avcpu_shm_log_init(void)
{
	dev_t dev;
	...
	np = of_find_compatible_node(NULL, NULL, HWSEM_COMPAT_NAME);
	if (!np) {
		pr_info("%s: no hw-semaphore match, skip\n", __func__);
		return 0;
	}
...
static void __exit rtk_avcpu_shm_log_exit(void)
{
	avlog_ctl_t *ctl = avlog_ctl_g;
...
late_initcall(rtk_avcpu_shm_log_init);
module_exit(rtk_avcpu_shm_log_exit);
