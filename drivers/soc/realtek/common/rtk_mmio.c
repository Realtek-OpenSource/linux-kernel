...
static struct rtk_secure_regfield crt_regs[] = {
	{ .offset = 0x030, .wcmd = 0x8400ff0b, .rcmd = 0x8400ff18, },
	{ .offset = 0x504, .wcmd = 0x8400ff0c, .rcmd = 0x8400ff19, },
};

static const struct rtk_regmap_config crt_config = {
	.addr = 0x98000000,
	.srfs = crt_regs,
	.num_srfs = ARRAY_SIZE(crt_regs),
	.config = {
		.reg_bits = 32,
		.val_bits = 32,
		.reg_stride = 4,
...
static struct rtk_mmio *create_rtk_mmio(struct device_node *np)
{
	...
	int ret;
	const struct rtk_regmap_config *conf;
