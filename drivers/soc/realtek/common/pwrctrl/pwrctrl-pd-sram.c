...
static int rtk_sram_power_init(struct power_control *pctrl);
...
static int rtk_sram_power_off_unused(struct power_control *pctrl)
{
	pr_info("%s: %s\n", pctrl->name, __func__);
	return pctrl->ops->power_off(pctrl);
}
...
static int rtk_sram_power_init(struct power_control *pctrl)
{
	...
	if ((spd->core.flags & RTK_PD_NO_GENPD) && pctrl->ops == &rtk_sram_async_ops) {
		...
		spd->timer.data = (unsigned long)pctrl;
