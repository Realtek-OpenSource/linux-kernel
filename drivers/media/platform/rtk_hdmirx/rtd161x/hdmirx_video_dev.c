...
static long v4l2_hdmi_do_ioctl(struct file *file, unsigned int cmd, void *arg)
{
	...
	case VIDIOC_GET_INFOFRAME:
	{
		...
		if ((info_packet->type == 0x84) && hdmirx_state.audio_detect_done) {
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (ct_index != 0xF) {
		ret_count += sprintf(buf+ret_count, "Coding Type:%s\n", coding_type[ct_index]);
	} else {
	...
	ret_count += sprintf(buf+ret_count, "Channel Count:%s\n", channel_count[cc_index]);

	sf_index = hdmi_rx.audiopkt_t.SF & 0x7;
	ret_count += sprintf(buf+ret_count, "Sampling Freq:%s\n", sampling_freq[sf_index]);

	ss_index = hdmi_rx.audiopkt_t.SS & 0x3;
	ret_count += sprintf(buf+ret_count, "Sampling Size:%s\n", sampling_size[ss_index]);
